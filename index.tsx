...
const RNNaverMapView =
  requireNativeComponent<NaverMapViewProps>("RNNaverMapView");
const RNNaverMapViewTexture = Platform.select({
  android: () =>
    requireNativeComponent<NaverMapViewProps>("RNNaverMapViewTexture"),
  ios: () => RNNaverMapView,
...
const RNNaverMapMarker = requireNativeComponent<{
  ...
  subCaption?: Omit<
    MarkerProps["subCaption"],
    "textSize" | "color" | "haloColor"
  > & {
    textSize: number;
    color: string | ProcessedColorValue | null | undefined;
    haloColor: string | ProcessedColorValue | null | undefined;
...
const RNNaverMapPathOverlay = requireNativeComponent<
  Omit<PathProps, "pattern"> & { pattern: string | null }
>("RNNaverMapPathOverlay");
const RNNaverMapPolylineOverlay = requireNativeComponent(
  "RNNaverMapPolylineOverlay"
);
const RNNaverMapCircleOverlay = requireNativeComponent(
  "RNNaverMapCircleOverlay"
);
const RNNaverMapPolygonOverlay = requireNativeComponent<
  Omit<PolygonProps, "coordinates"> & {
    coordinates:
      | Coord[]
      | {
          exteriorRing: Coord[];
...
function parseColor(
  color?: string | null
): string | ProcessedColorValue | null | undefined {
  if (color && Platform.OS === "ios") return processColor(color);
  return color;
}
